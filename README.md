# ERAtw-NEXT

ERAtw 现代化引擎与桌面应用（modernization engine & desktop app）。**不是**旧 Emuera/ERB 运行时的再打包。

长期方向是新 schema、新 engine、新 UI 和独立内容包；旧内容只作为外部只读参考。

## 当前阶段：M0-M4 已完成

- 技术栈：Rust workspace + Tauri 2 桌面壳 + React/TypeScript + MUI + Vite。
- 引擎 `eratw_next_engine` 提供 `system_get_status` 与 `map_get_overview`（纯查询，不读盘）。
- M1 工具 `eratw_next_content_audit` 提供只读审计 CLI：读取文本做聚合统计、读取资源计算 hash，但不输出正文、不修改源文件、不执行脚本、不访问网络。
- M2 工具 `eratw_next_content_migrate` 仅消费 M1 报告，在仓库外生成不可玩的 draft 内容包，并执行 schema、唯一 ID、引用和来源追溯校验。
- M3 engine loader 对仓库外内容包执行 schema、版本、依赖、路径和跨引用校验，桌面端可选择目录并浏览角色/地点/资源索引。
- M4 提供确定性 command reducer、时间与事件队列、版本化存档、依赖/hash/replay 校验。
- 首屏（status）展示项目身份、当前里程碑、能力列表与路径占位。
- 地图（map）支持**运行时随时切换两种显示方式**：
  - **字符画地图**：等宽字符网格 + 连线 + 节点字形 + 标签浮层 + 占用着色 + 缩放 + 键盘方向导航。
  - **SVG 地图**：节点圆形 + 连线 + MUI 浮动标签 + 缩放。
  - 两种模式共享同一份地图数据与选中/区域状态，切换不丢状态。
- JSON Schema 契约：`schemas/system-status.schema.json`、`schemas/map-model.schema.json`，由前端 Ajv 测试实际校验。
- 内容迁移契约：`schemas/content-audit-*.schema.json`、`schemas/content-package.schema.json`、`schemas/content-*.schema.json`、`schemas/migration-report.schema.json`。
- 运行时契约：`schemas/content-package-index.schema.json`、`schemas/game-*.schema.json`、`schemas/save-*.schema.json`。

> 地图数据为引擎内置的**自有示例数据**（幻想乡题材、自有布局），**不复制** `eratw-content` 的地图。真实数据后续由内容包提供。

## 安装与验证

```powershell
# Node.js 20.19+ / Rust stable
npm install
npm run typecheck
npm test
npm run build
npm run dev:web      # 浏览器预览（使用 mockData）
```

Rust / Tauri 验证（需本地 Rust 工具链；桌面构建在 Windows 上进行）：

```powershell
cargo fmt --check
cargo test --workspace
npm run dev          # Tauri 开发（需 Rust 工具链）
```

M3/M4 仓库外运行时验收：

```powershell
# 加载 M2 草案包，只建立索引
cargo run -p eratw_next_engine --example runtime_smoke -- `
  D:\AICODE\ERAtw-NEXT-artifacts\m2-draft-20260613-v4

# 生成仓库外自有最小包，执行命令并完成存档 round trip
cargo run -p eratw_next_engine --example runtime_smoke -- `
  --create-demo D:\AICODE\ERAtw-NEXT-artifacts\m4-playable-demo `
  D:\AICODE\ERAtw-NEXT-artifacts\m4-save.json
```

M1 只读内容审计（生成物默认位于 `reports/content-audit/`，不入库；`--out` 必须是源目录外的新路径）：

```powershell
cargo run -p eratw_next_content_audit -- `
  --source D:\AICODE\eratw-content `
  --out reports\content-audit\manual `
  --profile m1-readonly `
  --no-network --no-execute
```

M2 draft 内容包生成（只读取 M1 报告，输出目录必须位于本仓库和审计输入外，且不能已存在）：

```powershell
cargo run -p eratw_next_content_migrate -- `
  --audit D:\AICODE\ERAtw-NEXT\reports\content-audit\manual `
  --out D:\AICODE\ERAtw-NEXT-artifacts\content-package
```

真实数据验收结果：M1 扫描 6586 个文件；M2 生成 4194 个唯一对象和 2553 个显式未映射项，结构与交叉引用校验为有效。生成物不入库。

> 在浏览器中运行前端时（非 Tauri 环境），引擎客户端自动回退到镜像数据 `src/engine/mockData.ts`，
> 因此 `npm run dev`/`npm run build` 出的前端可独立预览地图与首屏。

## 目录结构

```text
ERAtw-NEXT/
  Cargo.toml                 # Rust workspace
  package.json               # npm workspaces
  schemas/                   # JSON Schema 契约（单一来源）
  crates/engine/             # eratw_next_engine：查询、内容运行时、状态机与存档
    src/content.rs           # M3 内容包只读加载、schema 与引用校验
    src/game.rs              # M4 状态机、时间、事件队列、reducer/replay
    src/save.rs              # M4 版本化存档与完整性校验
  tools/content-audit/        # M1 只读内容审计 CLI
  tools/content-migrate/      # M2 draft 内容包生成与验证 CLI
  apps/desktop/              # @eratw-next/desktop
    src/                     # React + MUI 前端
      engine/                # 客户端 + 镜像数据
      system/                # 系统状态首屏
      map/                   # 双模式地图（geometry / ascii / svg / view）
      content/               # 内容包选择与索引浏览
      game/                  # 游戏状态、命令与存档面板
      settings/              # 设置 context（地图显示方式）
    src-tauri/               # Tauri 壳与 command 桥接
  .github/workflows/ci.yml   # Windows 完整 / Ubuntu 非桌面
  docs/                      # 路线与设计文档
```

## 内容边界

- `D:\AICODE\eratw-content`：外部**只读**内容源，永不复制进本仓库。
- `D:\AICODE\eratw`：可游玩对照版本，仅供人工参考，引擎不读取。
- `D:\AICODE\ERAtw-modern`、`D:\AICODE\ERAtw-native-foundation`：**无关项目**，不作为输入、依赖或迁移来源。

## 安全边界

默认禁止执行外部 ERB / 脚本、禁止下载未知脚本、禁止自动访问未知 URL。
内容审计与 allowlist/路径穿越防护在 M1 细化。

## 路线

详见 [docs/roadmap/modernization-roadmap.md](docs/roadmap/modernization-roadmap.md)。
M0 仅建立可信工程基线（本阶段额外提前接入了地图功能）；M1 起进入只读内容审计与内容迁移。

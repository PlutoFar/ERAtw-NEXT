...
M2 定义新内容包契约（content package contract）和只生成草案的转换策略。它承接 M1 只读审计结果，但不实现运行时加载、不接 UI、不修改 M0 工程骨架、不把 `eratw-content` 内容本体写入 `ERAtw-NEXT`。
...
- 不做 ERB 解释器。
- 不做 UI。
- 不接入 `apps/`、`crates/`、root `Cargo.toml`、root `package.json`。
- 不把转换产物提交到 engine 仓库。
...
M2 只定义结构，不在 engine 仓库保存转换产物。真实输出应进入临时目录或独立内容包仓库。
...
M0/M1 合并后，为 M2 CLI/schema 实现单独写 implementation plan。首个实现不应生成真实内容包，只生成 schema 文件、fixture 和 validation tests。转换器实现应等 M1 CLI 的 source file index 稳定后再做。

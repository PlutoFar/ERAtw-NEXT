...
- Tauri `engine_discover_mods` / `engine_plan_mod_install` / `engine_install_mod` / `engine_plan_enabled_mods`：桌面层把发现报告、安装计划、安装结果和启用计划转换成前端稳定 DTO，包含可展示的错误类型和消息。

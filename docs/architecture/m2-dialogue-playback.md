...
- `eratw_content::ContentPackage`：封装 manifest、ResourceAsset、DialogueScene 与 ScheduledEvent 列表。
- `ContentPackage::validate`：在运行前报告资源元数据缺失、入口缺失、死链、重复 ID、不可达节点、非法调度事件等问题。
- `ContentPackage::install_into_world`：只安装校验干净且不与现有世界 ID 冲突的 ResourceAsset/DialogueScene/ScheduledEvent。

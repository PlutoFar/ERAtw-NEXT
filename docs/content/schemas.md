...
- ResourceAsset 空 ID、重复 ID、空 source_path、不安全 source_path、空/unknown license、空/unknown author。
- DialogueScene 空 ID、重复场景 ID。
- DialogueNode 空 ID、重复节点 ID、空文本。
- DialogueNode 空资源引用。
- Dialogue 文本、选择标签和 AddLog 文本中的占位符语法错误、未知变量或显式类型不匹配。
- 入口节点不存在。
- Choice 指向不存在的下一节点。
- Choice 条件引用空 ID 或非法时间。
- Choice 随机状态效果的最小增量大于最大增量。
- ScheduledEvent 随机状态事件的最小增量大于最大增量。
- 从入口节点不可达的节点。
...
- 剧情文本占位符使用 `{{variable}}` 或 `{{variable:type}}`。当前变量白名单：
  `speaker.id`、`speaker.name`、`speaker.location_id`、`scene.id`、`node.id`、
  `player.id`、`clock.season`、`clock.weather` 为 `text/string`；
  `speaker.energy`、`speaker.mood`、`clock.day`、`clock.hour`、`clock.minute` 为
  `number`。
- `source_path` 必须是内容包根目录下的相对路径，不允许绝对路径、盘符路径或 `..` 逃逸。
